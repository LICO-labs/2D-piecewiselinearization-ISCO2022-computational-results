# Copyright (c) 2022: Alois Duguet
#
# created by Alois Duguet
...
function parse_and_plot_instance(method_name, function_name, delta)
    ...
    INPUT:
    method_name (string): name of the method used for the resolution of the CFP instance. Either "DN95" or "DN99"
    ...
    filename = joinpath("rawlogs", method_name * ".txt")
    @assert isfile(filename) "file $(filename) does not exist"
    println("\t ... read file $(filename) ... ok")
    ...
    @assert haskey(real_func_names, function_name) "function name $(function_name) is not among the list considered that is : $(keys(real_func_names))"
    println("\t ... get function name : $(function_name) ... ok")
    ...
    for line in lines
        ...
        if res1 != nothing && res2 != nothing
            ...
            delta = err.delta
            line_found = true
            println("\t ... get result for error value $(delta) ... ok")
            break
        end
    end
    ...
    if line_found
        ...
        println("\t ... save solution figure ''", name_figure, "'' in folder images/ ... ok " )
        savefig(joinpath("images",name_figure)) # save figure with name name_figure
        display(p)
    else
        error("the result on instance ($function_name, $delta) with method $method_name was not found")
    end
